<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-2xl font-bold text-brand-900">学员进度</h1>
      <p class="text-brand-400 mt-1">查看和管理学员学习进度</p>
    </div>

    <div class="flex flex-wrap gap-4">
      <div class="relative flex-1 min-w-[200px] max-w-sm">
        <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-400" />
    ...
    <div class="bg-white rounded-xl border border-brand-400/20 shadow-sm overflow-hidden">
        ...
        <table class="w-full">
          <thead>
            <tr class="bg-brand-400/10 border-b border-brand-400/20">
              <th class="text-left px-6 py-3 text-sm font-medium text-brand-400">学员</th>
              <th class="text-left px-6 py-3 text-sm font-medium text-brand-400">课程</th>
              <th class="text-left px-6 py-3 text-sm font-medium text-brand-400">状态</th>
              <th class="text-left px-6 py-3 text-sm font-medium text-brand-400">学习进度</th>
              <th class="text-left px-6 py-3 text-sm font-medium text-brand-400">操作</th>
            </tr>
          </thead>
            ...
            <tr v-for="enr in displayEnrollments" :key="enr.id" class="hover:bg-brand-400/10 transition-colors">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <span class="font-medium text-brand-900">{{ getStudentName(enr.studentId) }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-sm text-brand-600">{{ getCourseTitle(enr.courseId) }}</td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-xs font-medium text-brand-600 w-10">{{ enr.progress }}%</span>
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-1">
                  <button @click="handleProgressUpdate(enr.id, enr.progress - 10)"
                    class="px-2 py-1 text-xs text-brand-400 hover:bg-brand-400/10 rounded transition-colors">-10</button>
                    <button @click="handleProgressUpdate(enr.id, enr.progress + 10)"
                    class="px-2 py-1 text-xs text-brand-400 hover:bg-brand-400/10 rounded transition-colors">+10</button>
                </div>
              </td>
      ...
      <div v-if="displayEnrollments.length === 0" class="text-center py-8 text-brand-400">暂无数据</div>
    </div>
  </div>
</template>
...
const statusColors: Record<string, string> = {
  enrolled: 'bg-brand-600/10 text-brand-600', in_progress: 'bg-brand-400/10 text-brand-600',
  completed: 'bg-brand-400/10 text-brand-600', dropped: 'bg-brand-600/10 text-brand-600',
}
