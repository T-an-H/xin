...
function renderAdminStudents(root: HTMLElement) {
  ...
  header.append('h1').attr('class', 'text-2xl font-bold text-gray-900').text('学员管理')
  header.append('p').attr('class', 'text-gray-400 mt-1').text('管理所有学员信息')

  // ---- 搜索栏 ----
  const filterRow = wrapper.append('div').attr('class', 'flex gap-3')
  const searchWrap = filterRow.append('div').attr('class', 'relative flex-1')
  renderIcon(searchWrap, 'search', 'w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400')
  ...
  headRow.append('th').attr('class', 'text-left px-4 py-3 text-xs font-medium text-gray-400 uppercase').text('姓名')
  headRow.append('th').attr('class', 'text-left px-4 py-3 text-xs font-medium text-gray-400 uppercase').text('学号')
  headRow.append('th').attr('class', 'text-left px-4 py-3 text-xs font-medium text-gray-400 uppercase').text('班级')
  headRow.append('th').attr('class', 'text-left px-4 py-3 text-xs font-medium text-gray-400 uppercase').text('已选课程')
  headRow.append('th').attr('class', 'text-right px-4 py-3 text-xs font-medium text-gray-400 uppercase').text('操作')
  ...
  students.forEach((s) => {
    ...
    // 已选课程
    row.append('td').attr('class', 'px-4 py-3 text-sm text-gray-600').text(`${getEnrolledCount(s.id)} 门`)

    // 操作
    const actionCell = row.append('td').attr('class', 'px-4 py-3 text-right')
    actionCell.append('div')
      .attr('class', 'text-xs text-gray-600 hover:underline cursor-pointer')
  ...
  if (totalPages.value > 1) {
    ...
    pagination.append('span').attr('class', 'text-sm text-gray-400').text(`共 ${store.students.length} 名学员`)
    ...
    for (let p = 1; p <= totalPages.value; p++) {
      ...
      pageBtns.append('button')
        .attr('class', `px-3 py-1.5 rounded-lg text-sm ${isCurrent ? 'bg-brand-600 text-white' : 'bg-brand-400/10 text-gray-600 hover:bg-brand-400/10'}`)
