...
function renderLeaderCourses(root: HTMLElement) {
  ...
  header.append('h1').attr('class', 'text-2xl font-bold text-gray-900').text('课程总览')
  header.append('p').attr('class', 'text-gray-400 mt-1').text('查看管辖学院的所有课程信息')
  ...
  totalCard.append('div').attr('class', 'w-10 h-10 rounded-lg bg-brand-600/10 flex items-center justify-center').call((sel) => renderIcon(sel, 'bookOpen', 'w-5 h-5 text-gray-600'))
  const totalInfo = totalCard.append('div')
  totalInfo.append('p').attr('class', 'text-xs text-gray-400').text('总课程数')
  totalInfo.append('p').attr('class', 'text-xl font-bold text-gray-900').text(String(courses.value.length))
  ...
  activeCard.append('div').attr('class', 'w-10 h-10 rounded-lg bg-brand-600/10 flex items-center justify-center').call((sel) => renderIcon(sel, 'play', 'w-5 h-5 text-gray-600'))
  const activeInfo = activeCard.append('div')
  activeInfo.append('p').attr('class', 'text-xs text-gray-400').text('进行中')
  ...
  inactiveCard.append('div').attr('class', 'w-10 h-10 rounded-lg bg-brand-400/10 flex items-center justify-center').call((sel) => renderIcon(sel, 'checkCircle', 'w-5 h-5 text-gray-400'))
  const inactiveInfo = inactiveCard.append('div')
  inactiveInfo.append('p').attr('class', 'text-xs text-gray-400').text('已结束')
  inactiveInfo.append('p').attr('class', 'text-xl font-bold text-gray-900').text(String(inactiveCount.value))

  // Category sections
  const catList = categories.value
  if (catList.length === 0) {
    const emptyCard = container.append('div').attr('class', 'bg-white rounded-xl border border-brand-400/20 shadow-sm p-12 text-center text-gray-400')
  ...
  catList.forEach((cat) => {
    ...
    catHeader.append('h2').attr('class', 'text-base font-semibold text-gray-900').text(cat.name)
    catHeader.append('span').attr('class', 'text-xs text-gray-400 ml-auto').text(`${getCategoryCourseCount(cat.id)} 门课程`)
    ...
    if (catCourses.length === 0) {
      courseList.append('div').attr('class', 'px-5 py-8 text-center text-sm text-gray-400').text('暂无课程')
    } else {
      catCourses.forEach((course) => {
        ...
        titleRow.append('h3').attr('class', 'text-sm font-medium text-gray-900 truncate').text(course.title)

        let statusLabel = ''
        let statusClass = ''
        if (course.status === 'active') {
          statusLabel = '进行中'
          statusClass = 'bg-brand-600/10 text-gray-600'
        } else if (course.status === 'inactive') {
          statusLabel = '已结束'
          statusClass = 'bg-brand-400/10 text-gray-400'
        } else {
          statusLabel = '草稿'
          statusClass = 'bg-brand-400/10 text-gray-400'
        }
        ...
        // Meta info
        const metaRow = leftCol.append('div').attr('class', 'flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-400')
