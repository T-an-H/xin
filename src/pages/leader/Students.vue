...
function renderStudents(root: HTMLElement) {
  ...
  header.append('h1').attr('class', 'text-2xl font-bold text-brand-900').text('学员总览')
  header.append('p').attr('class', 'text-brand-400 mt-1').text('查看管辖学院的所有学员信息')
  ...
  totalCard.append('div').attr('class', 'w-10 h-10 rounded-lg bg-brand-600/10 flex items-center justify-center')
    .call((sel) => renderIcon(sel, 'users', 'w-5 h-5 text-brand-600'))
  const totalInfo = totalCard.append('div')
  totalInfo.append('p').attr('class', 'text-xs text-brand-400').text('总学员数')
  totalInfo.append('p').attr('class', 'text-xl font-bold text-brand-900').text(String(students.value.length))
  ...
  activeCard.append('div').attr('class', 'w-10 h-10 rounded-lg bg-brand-600/10 flex items-center justify-center')
    .call((sel) => renderIcon(sel, 'userCheck', 'w-5 h-5 text-brand-600'))
  const activeInfo = activeCard.append('div')
  activeInfo.append('p').attr('class', 'text-xs text-brand-400').text('活跃学员')
  ...
  headers.forEach((h) => {
    headerRow.append('th').attr('class', 'text-left px-4 py-3 text-xs font-medium text-brand-400 uppercase').text(h)
  })
  ...
  if (studentList.length === 0) {
    ...
    studentList.forEach((s) => {
      ...
      row.append('td').attr('class', 'px-4 py-3 text-sm text-brand-600').text(s.phone)
      row.append('td').attr('class', 'px-4 py-3 text-sm text-brand-600').text(s.email)
      row.append('td').attr('class', 'px-4 py-3 text-sm text-brand-600').text(s.enrollmentScore ?? '-')
