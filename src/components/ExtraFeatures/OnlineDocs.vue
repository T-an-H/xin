<template>
  <div class="space-y-4">
    <div class="flex items-center gap-3">
      <input type="text" v-model="newTitle" @keydown.enter="handleCreate" placeholder="新建文档标题..." class="flex-1 px-4 py-2.5 rounded-lg border border-gray-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none text-sm" />
    ...
    <div v-for="doc in myDocs" :key="doc.id" class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      <div v-if="editingId === doc.id" class="p-4 space-y-3">
        <input type="text" v-model="editTitle" class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-purple-500 outline-none text-sm font-medium" />
        <textarea v-model="editContent" rows="6" class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-purple-500 outline-none text-sm resize-y" placeholder="在此输入文档内容..."></textarea>
        <div class="flex gap-2">
          <button @click="handleSave(doc.id)" class="flex items-center gap-1.5 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-colors text-sm">
      ...
      <div v-else class="p-4">
        <div class="flex items-start justify-between mb-2">
          ...
          <div class="flex gap-1">
            <button @click="startEdit(doc)" class="p-1.5 rounded-lg hover:bg-purple-50 text-purple-500 transition-colors" title="编辑">
              <Edit3 class="w-4 h-4" />
            </button>
            <button @click="store.deleteOnlineDoc(doc.id)" class="p-1.5 rounded-lg hover:bg-red-50 text-red-400 transition-colors" title="删除">
