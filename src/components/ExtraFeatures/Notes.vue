    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="note in myNotes" :key="note.id" class="bg-white rounded-xl border border-brand-400/20 shadow-sm overflow-hidden">
        <div v-if="editingId === note.id" class="p-4 space-y-3">
          <input type="text" v-model="editTitle" class="w-full px-3 py-2 rounded-lg border border-brand-400/30 focus:border-emerald-500 outline-none text-sm font-medium" />
          <textarea v-model="editContent" rows="5" class="w-full px-3 py-2 rounded-lg border border-brand-400/30 focus:border-emerald-500 outline-none text-sm resize-y" placeholder="在此输入笔记内容..."></textarea>
          <div class="flex gap-2">
            ...
            <button @click="editingId = null" class="px-4 py-2 bg-gray-100 hover:bg-brand-400/10 text-gray-600 rounded-lg transition-colors text-sm">取消</button>
          </div>
        </div>
